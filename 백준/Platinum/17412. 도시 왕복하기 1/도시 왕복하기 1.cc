...
int main (int argc, char* argv[]) {
	...
	int vertex = 7, edge = 11;
	cin >> vertex >> edge;
	
	struct NodeType* node = new struct NodeType[vertex + 1];
	if (node == NULL) throw;
	...
	for (int repeat = 0; repeat < edge; repeat++) {
		...
		node[from].Element.push_back({to, weight});
		node[to].Element.push_back({from, 0});
	}
	
	cout << NumberOfPath (node, 1, 2, vertex) - 1 << "\n";
	return 0;
}
